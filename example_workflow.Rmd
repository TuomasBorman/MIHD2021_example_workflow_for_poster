...
According to Lin H. & Peddada SD (2020) [Analysis of compositions of microbiomes 
with bias correction](https://doi.org/10.1038/s41467-020-17041-7), ANCOM-BC has
high power and low FDR even when proportion of differential abundant taxa is low.
...
knitr::kable(df, caption = paste0(" P-values of abundance in comp. vs. uncomp, 
                           number of samples where counts was found (total = ", dim(tse)[2], ") and 
...
# Add p-values
df <- dplyr::left_join(df, tibble::rownames_to_column(p_values), by = c("FeatureID" = "rowname"))
...
ggplot(df, aes(x = FeatureID, y = clr)) + theme_bw()+
  ...
  geom_point(aes(color = Diet), position=position_jitterdodge(jitter.width = 0.25), size = 1) +
  theme_bw(11) +
